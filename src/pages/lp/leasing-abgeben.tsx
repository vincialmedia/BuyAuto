  ...
  const heroChips =
    takeoverListings.length >= 3
      ? takeoverListings.slice(0, 3).map((listing) => ({
          ...
          label: t("{vehicle} · {price} / Monat", {
            vehicle: listing.title?.trim() || `${listing.brand} ${listing.model}`,
            price: fmtChf(listing.pricePerMonthCHF),
          }),
        }))
      : FALLBACK_CHIPS.map((chip) => ({ ...chip, label: t(chip.label) }));

  return (
    <>
      <Head>
        <title>{t("Leasing abgeben Schweiz: legal & ohne Verlust raus | BuyAuto")}</title>
        <meta
          name="description"
          content={t(
            "Leasing abgeben in der Schweiz leicht gemacht: Übergib deinen Leasingvertrag an einen Nachfolger und zahle nur die Umschreibegebühr.",
          )}
        />
      ...
      <div className="bg-white">
          ...
          <div className="bg-neutral-900 border-t border-white/10 shadow-2xl px-4 py-3">
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <p className="text-white text-sm font-bold leading-tight">{t("Bereit, dein Leasing abzugeben?")}</p>
                <p className="text-white/60 text-xs leading-tight mt-0.5">{t("Gratis inserieren · 60 Tage online")}</p>
              </div>
          ...
          <div className="max-w-[1120px] mx-auto px-5 sm:px-10 h-[68px] flex items-center justify-between overflow-visible">
            ...
            <div className="flex items-center gap-4">
              <span className="hidden lg:inline-flex items-center gap-2.5 text-[13px] font-semibold text-neutral-500">
                ...
                {t("100% legal – der Vertrag läuft weiter")}
              </span>
          ...
          <div className="relative max-w-[1120px] mx-auto px-5 sm:px-10 py-12 lg:py-[72px] grid lg:grid-cols-[1fr_460px] gap-10 lg:gap-14 items-center">
            ...
            <div>
              ...
              <h1 className="mt-[18px] text-[34px] sm:text-[42px] lg:text-[50px] leading-[1.05] font-black tracking-[-0.025em] text-white [hyphens:auto]">
                <T k="Leasing abgeben – <0>ohne teure Kündigung.</0>" c={[<span key="0" className="text-red-500" />]} />
              </h1>
              ...
              <div className="mt-8">
                <p className="mb-2.5 text-[11px] font-extrabold tracking-[0.08em] uppercase text-white/55">
                  {t("Live auf BuyAuto · Diese Fahrer geben gerade ihr Leasing ab")}
                </p>
              ...
              <div className="bg-white rounded-3xl p-7 shadow-[0_25px_60px_rgb(0_0_0/0.45)]">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h2 className="text-xl font-black tracking-[-0.02em] text-neutral-900">
                      {t("Was kostet dich der Ausstieg?")}
                    </h2>
                ...
                <div className="mt-6">
                  <div className="flex justify-between items-baseline mb-2.5">
                    <label className="text-[13.5px] font-bold text-neutral-700">{t("Restlaufzeit")}</label>
                    <span className="text-[15px] font-extrabold text-red-600">{t("{months} Monate", { months })}</span>
                  </div>
                  <Slider
                    value={[months]}
                ...
                <div className="mt-5">
                  <div className="flex justify-between items-baseline mb-2.5">
                    <label className="text-[13.5px] font-bold text-neutral-700">{t("Monatsrate")}</label>
                ...
                <div className="grid grid-cols-2 gap-3 mt-3">
                  <div className="relative bg-emerald-50 border border-emerald-200 rounded-[14px] px-4 py-3.5">
                    ...
                    <p className="text-[10.5px] font-extrabold tracking-[0.06em] uppercase text-emerald-700">{t("Übernahme")}</p>
                    <p className="mt-1 text-xl leading-tight font-black text-emerald-700 whitespace-nowrap">CHF 0 – 650</p>
                    <p className="mt-0.5 text-[11px] leading-tight text-emerald-600">{t("Umschreibegebühr")}</p>
                  </div>
                  <div className="bg-red-600 rounded-[14px] px-4 py-3.5">
                    <p className="text-[10.5px] font-extrabold tracking-[0.06em] uppercase text-white/70">{t("Kündigung")}</p>
                    <p className="mt-1 text-xl leading-tight font-black text-white">{t("Mehrere tausend")}</p>
                    <p className="mt-0.5 text-[11px] leading-tight text-white/70">{t("je nach Bank & Vertrag")}</p>
                  </div>
                </div>
                ...
                <p className="mt-3 text-[10.5px] leading-normal text-neutral-400">
                  {t(
                    "Richtwerte zur Orientierung. Massgebend sind dein Leasingvertrag und die Konditionen deiner Leasingbank.",
                  )}
                </p>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-3.5 mt-9">
            <div className="grid lg:grid-cols-[250px_1fr_190px] gap-5 lg:gap-6 items-center bg-white border-2 border-red-500 rounded-[20px] px-6 py-[22px] sm:px-7 shadow-[0_10px_30px_rgb(239_68_68/0.1)]">
              <div>
                ...
                <h3 className="mt-2.5 text-[19px] leading-tight font-extrabold text-neutral-900">{t("Leasingübernahme")}</h3>
                <p className="mt-1 text-xs leading-tight font-bold text-red-700">{t("BESTE LÖSUNG")}</p>
              </div>
              ...
              <div className="lg:text-right">
                ...
                <p className="mt-0.5 text-[11.5px] leading-tight text-neutral-500">{t("Umschreibegebühr")}</p>
              </div>
            </div>
            ...
            <div className="grid lg:grid-cols-[250px_1fr_190px] gap-5 lg:gap-6 items-center bg-white border border-neutral-200 rounded-[20px] px-6 py-[22px] sm:px-7">
              <div>
                ...
                <h3 className="mt-2.5 text-[19px] leading-tight font-extrabold text-neutral-900">{t("Vorzeitige Kündigung")}</h3>
              </div>
              <ul className="grid sm:grid-cols-2 gap-y-2 gap-x-5">
                {[
              ...
              <div className="lg:text-right">
                <p className="text-[17px] leading-tight font-extrabold text-neutral-900 whitespace-nowrap">{t("Mehrere tausend")}</p>
                <p className="mt-0.5 text-[11.5px] leading-tight text-neutral-500">{t("Fast immer die teuerste Lösung")}</p>
              </div>
            </div>
            ...
            <div className="grid lg:grid-cols-[250px_1fr_190px] gap-5 lg:gap-6 items-center bg-white border border-neutral-200 rounded-[20px] px-6 py-[22px] sm:px-7">
              <div>
                ...
                <h3 className="mt-2.5 text-[19px] leading-tight font-extrabold text-neutral-900">{t("Auto verkaufen")}</h3>
              </div>
              <ul className="grid sm:grid-cols-2 gap-y-2 gap-x-5">
                {[
              ...
              <div className="lg:text-right">
                <p className="text-[17px] leading-tight font-extrabold text-neutral-900">{t("Unsicher")}</p>
                <p className="mt-0.5 text-[11.5px] leading-tight text-neutral-500">{t("Nur bei hohem Marktwert sinnvoll")}</p>
              </div>
            </div>
          </div>
              ...
              <div>
                ...
                <p className="mt-2.5 text-[15px] text-neutral-600">
                  <T
                    ...
                    c={[<Link key="0" href="/leasingvertrag-uebertragen" className="font-semibold text-red-600 hover:text-red-700" />]}
                  />
                </p>
              </div>
              ...
              <ol className="grid lg:grid-cols-5 gap-y-8">
                {PROCESS_STEPS.map((item) => (
                  <li key={item.step} className="relative text-center px-3">
                    ...
                    <h3 className="mt-3 text-[14.5px] leading-tight font-extrabold text-neutral-900">{t(item.title)}</h3>
        ...
        <section id="faq" className="max-w-[1120px] mx-auto px-5 sm:px-10 py-12 lg:py-[72px] scroll-mt-20">
          ...
          <p className="mt-2.5 text-[15px] text-neutral-600">{t("Alles, was du wissen musst")}</p>
          <div className="grid md:grid-cols-2 gap-5 mt-8">
            {FAQS.map((faq) => (
              <div key={faq.q} className="bg-white border border-neutral-200 rounded-2xl p-6">
                <h3 className="text-base leading-snug font-extrabold text-neutral-900">{t(faq.q)}</h3>
          ...
          <div className="bg-red-600 rounded-[32px] px-8 py-10 sm:px-12 sm:py-14 flex flex-col md:flex-row items-center justify-between gap-10 shadow-[0_20px_45px_rgb(239_68_68/0.25)]">
            <div className="text-center md:text-left">
              <h2 className="text-3xl sm:text-4xl font-black tracking-[-0.025em] leading-[1.1] text-white">
                {t("Gib dein Leasing ab – legal, schnell & günstig")}
              </h2>
            ...
            <div className="flex-none text-center">
              ...
              />
              <p className="mt-2.5 text-[12.5px] text-white/75">{t("Gratis · 60 Tage online")}</p>
            </div>
          </div>
        </section>
      </div>
...
export const getStaticProps: GetStaticProps<LeasingAbgebenPageProps> = async (context) => {
  const i18n = await withI18n(context.locale, ["pages/lp-leasing-abgeben"]);
  try {
