...
import { T, useLocale, useT, type TranslateVars } from "@/i18n/runtime";
...
const FAQ_ITEMS: Array<{ q: string; a: string; vars?: TranslateVars }> = [
  {
  ...
  },
  {
    ...
    // The German text is the translation key; `vars` fills its placeholders.
    a: "Die ersten {free} automatischen Suchen pro Monat sind gratis. Danach brauchst du ein Garagen-Paket: Starter {starter}, Growth {growth} und Pro {pro} automatische Suchen pro Monat. Manuelle Berechnungen (eigene Vergleichspreise eintragen) sind in jedem Paket unbegrenzt gratis.",
    vars: {
...
export async function getStaticProps({ locale }: GetStaticPropsContext) {
  return {
    props: {
      ...
      ...(await withI18n(locale, ["calculator", "pages/eintauschwert-rechner"])),
    },
    revalidate: 86400,
  };
}
...
export default function EintauschwertRechnerPage({ updatedDate }: PageProps) {
  ...
  const canonical = absoluteUrl("/eintauschwert-rechner", locale);
  ...
  const faqSchema = {
    ...
    mainEntity: FAQ_ITEMS.map((f) => ({
      "@type": "Question",
      name: t(f.q),
      acceptedAnswer: { "@type": "Answer", text: t(f.a, f.vars) },
    })),
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: t("Für Garagen"), item: absoluteUrl("/garage-plan", locale) },
      { "@type": "ListItem", position: 3, name: t("Eintauschwert-Rechner"), item: canonical },
    ],
  };
  ...
  return (
    <>
      <Head>
        <title>{t("Eintauschwert berechnen (Schweiz): Ankaufsrechner für Garagen | BuyAuto")}</title>
        <meta
          name="description"
          content={t(
            "Eintauschwert & Ankaufspreis in 2 Minuten berechnen: Marktwert aus echten Schweizer Vergleichsinseraten, minus Aufbereitung, Garantie, Standzeit & Marge. Gratis für Garagen & Händler."
          )}
        />
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              "@context": "https://schema.org",
              "@type": "Article",
              headline: t("Eintauschwert berechnen: der Ankaufsrechner für Garagen in der Schweiz"),
        ...
        {/* Open Graph */}
        <meta property="og:title" content={t("Eintauschwert berechnen (Schweiz): Ankaufsrechner für Garagen")} />
        <meta
          property="og:description"
          content={t(
            "Marktwert aus echten Schweizer Vergleichsinseraten, minus Aufbereitung, Garantie, Standzeit & Marge. Gratis für Garagen & Händler."
          )}
        />
      ...
      <main className="bg-neutral-50 min-h-screen">
        ...
        <section className="bg-gradient-to-b from-neutral-900 to-neutral-800 text-white pt-20 pb-8 px-4">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <nav aria-label={t("Brotkrumen")} className="mb-5 flex items-center justify-center gap-1.5 text-xs text-neutral-400">
              <Link href="/" className="hover:text-white transition-colors">{t("Home")}</Link>
              <ChevronRight className="w-3 h-3" aria-hidden="true" />
              <Link href="/garage-plan" className="hover:text-white transition-colors">{t("Für Garagen")}</Link>
              <ChevronRight className="w-3 h-3" aria-hidden="true" />
              <span className="text-neutral-200">{t("Eintauschwert-Rechner")}</span>
            </nav>
            ...
            <div className="inline-flex items-center gap-2 bg-red-600 text-white px-4 py-1.5 rounded-full text-xs font-semibold mb-5">
              ...
              <span>{t("Ankaufs-Rechner · Aktualisiert am {date}", { date: updatedDate })}</span>
            </div>
            ...
            <p className="text-lg md:text-xl text-neutral-300 leading-relaxed mb-6 max-w-2xl mx-auto">
              {t("Der Eintauschwert ist der Preis, den die Garage beim Eintausch zahlt: Marktwert minus Aufbereitung, Garantie-Rückstellung, Standzeit und Marge – als Faustregel 80–90% des Marktwerts. Gib Marke, Modell, Jahrgang und Kilometerstand ein; der Rechner zieht echte Vergleichsinserate bei und liefert in 2 Minuten den Ankaufspreis samt Rechenweg fürs Kundengespräch.")}
            </p>

            {/* Trust row */}
            <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-sm text-neutral-300">
              <span className="flex items-center gap-1.5"><Check className="w-4 h-4 text-red-400" /> {t("Kostenlos starten")}</span>
              <span className="flex items-center gap-1.5"><Check className="w-4 h-4 text-red-400" /> {t("Ohne Anmeldung testen")}</span>
              <span className="flex items-center gap-1.5"><Check className="w-4 h-4 text-red-400" /> {t("Echte CH-Inseratspreise")}</span>
            </div>
          </div>
        </section>
        ...
        <section id="so-funktionierts" className="py-16 px-4 bg-neutral-50 scroll-mt-20">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-2xl md:text-3xl font-bold text-neutral-900 mb-8 flex items-center gap-3">
              ...
              {t("So funktioniert die Eintauschwert-Berechnung")}
            </h2>
            <ol className="grid grid-cols-1 md:grid-cols-3 gap-4 list-none pl-0">
              <li className="bg-white p-5 rounded-xl border border-neutral-200 shadow-sm">
                ...
                <p className="mt-2 text-neutral-700"><T k="<0>Fahrzeug eingeben:</0> Marke, Modell, Jahrgang, Kilometerstand. Der Rechner sucht automatisch bis zu 5 passende Inserate – oder du erfasst sie manuell." c={[<strong key="0" />]} /></p>
              </li>
              <li className="bg-white p-5 rounded-xl border border-neutral-200 shadow-sm">
                <span className="text-red-600 font-bold text-2xl">2.</span>
                <p className="mt-2 text-neutral-700"><T k="<0>Abzüge eintragen:</0> Aufbereitung, Garantie-Rückstellung, Standzeit und deine Marge – die Vorschläge kannst du überschreiben." c={[<strong key="0" />]} /></p>
              </li>
              <li className="bg-white p-5 rounded-xl border border-neutral-200 shadow-sm">
                <span className="text-red-600 font-bold text-2xl">3.</span>
                <p className="mt-2 text-neutral-700"><T k="<0>Ankaufspreis erhalten:</0> Marktwert, transparenter Rechenweg und gerundetes Angebot – bereit fürs Kundengespräch." c={[<strong key="0" />]} /></p>
              </li>
            </ol>
          </div>
        </section>
        ...
        <section id="methode" className="py-16 px-4 bg-white scroll-mt-20">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-2xl md:text-3xl font-bold text-neutral-900 mb-6">
              {t("Wie wird der Eintauschwert berechnet?")}
            </h2>
            <div className="bg-red-50 border-l-4 border-red-600 p-6 rounded-r-xl mb-6">
              <p className="text-lg text-neutral-800 font-semibold">
                {t("Eintauschwert = Marktwert − Aufbereitung − Garantie-Rückstellung − Standzeit − Marge")}
              </p>
            </div>
            <div className="prose prose-lg text-neutral-700 max-w-none">
              <p>
                <T
                  k="Der <0>Marktwert</0> ist der Median von 3–5 vergleichbaren Inseraten. Hat ein Vergleichsauto mehr oder weniger Kilometer als deins, wird sein Preis automatisch angeglichen – rund 5% des Inseratspreises pro 10'000 km Differenz (bei einem CHF 20'000-Auto sind 20'000 km ≈ CHF 2'000). Der Median statt des Durchschnitts sorgt dafür, dass ein einzelnes überteuertes oder verschleudertes Inserat das Ergebnis nicht verzerrt. Und wenn die Datenlage dünn ist – weniger als 3 Inserate oder stark streuende Preise – zeigt der Rechner ehrlich eine Spanne statt eines einzelnen Werts."
                  c={[<strong key="0" />]}
                />
              </p>
              <p>
                <T
                  k="Davon ziehst du deine <0>echten Kosten</0> ab (Aufbereitung 300–1'500 CHF, Garantie-Rückstellung 300–800 CHF, Standzeit & Kapitalbindung) sowie deine <1>Marge</1> (branchenüblich 10–20% vom Marktwert oder ein fixes Ertragsziel). Übrig bleibt ein fairer, verteidigbarer Ankaufspreis – den Rechenweg kannst du dem Kunden offen zeigen."
        ...
        <section id="faktoren" className="py-16 px-4 bg-neutral-50 scroll-mt-20">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-2xl md:text-3xl font-bold text-neutral-900 mb-8 flex items-center gap-3">
              ...
              {t("Welche Faktoren beeinflussen den Eintauschwert?")}
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {FACTORS.map((f) => (
                <div key={f.label} className="bg-white p-5 rounded-xl border border-neutral-200 shadow-sm">
                  <h3 className="font-bold text-neutral-900 mb-1">{t(f.label)}</h3>
                  <p className="text-sm text-neutral-600 leading-relaxed">{t(f.text)}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section id="prozent-oder-fix" className="py-16 px-4 bg-white scroll-mt-20">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-2xl md:text-3xl font-bold text-neutral-900 mb-8 flex items-center gap-3">
              ...
              {t("Marge: Prozent oder Fixbetrag?")}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-neutral-50 rounded-xl p-8 border border-neutral-200">
                <h3 className="text-xl font-bold text-neutral-900 mb-4 flex items-center gap-2">
                  <Percent className="w-6 h-6 text-red-600" />
                  {t("Prozent vom Marktwert")}
                </h3>
                <ul className="space-y-3">
                  <li className="flex gap-3">
                    <Check className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
                    <span className="text-neutral-700"><T k="<0>Branchenüblich: 10–20%.</0> Skaliert automatisch mit dem Fahrzeugwert." c={[<strong key="0" />]} /></span>
                  </li>
                  <li className="flex gap-3">
                    <Check className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
                    <span className="text-neutral-700">{t("Sinnvoll ab ca. 10'000 CHF Fahrzeugwert – die Marge deckt Risiko und Aufwand proportional.")}</span>
                  </li>
                  <li className="flex gap-3">
                    <Check className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
                    <span className="text-neutral-700">{t("Vorsicht bei teuren Fahrzeugen: 15% von 60'000 CHF sind 9'000 CHF – das Angebot wird schnell unattraktiv.")}</span>
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-neutral-50 rounded-xl p-8 border border-neutral-200">
                <h3 className="text-xl font-bold text-neutral-900 mb-4 flex items-center gap-2">
                  ...
                  {t("Fixbetrag pro Fahrzeug")}
                </h3>
                <ul className="space-y-3">
                  <li className="flex gap-3">
                    <Check className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
                    <span className="text-neutral-700"><T k="<0>Ertragsziel pro Auto</0> (z.B. 1'500–2'500 CHF) – einfach zu kalkulieren und zu kontrollieren." c={[<strong key="0" />]} /></span>
                  </li>
                  <li className="flex gap-3">
                    <Check className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
                    <span className="text-neutral-700">{t("Besser bei günstigen Occasionen: 12% von 8'000 CHF wären nur 960 CHF – zu wenig für den gleichen Aufwand.")}</span>
                  </li>
                  <li className="flex gap-3">
                    <Check className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
                    <span className="text-neutral-700">{t("Kombinier-Tipp: Fixbetrag als Untergrenze, Prozent als Standard.")}</span>
                  </li>
                </ul>
              </div>
            </div>
        ...
        <section id="vs-verkaufswert" className="py-16 px-4 bg-neutral-50 scroll-mt-20">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-2xl md:text-3xl font-bold text-neutral-900 mb-6">
              {t("Eintauschwert vs. Verkaufswert")}
            </h2>
            <div className="prose prose-lg text-neutral-700 max-w-none">
              <p>
                <T
                  k="Der <0>Verkaufswert</0> (Marktwert) ist der Preis, zu dem das Fahrzeug am Markt inseriert wird. Der <1>Eintauschwert</1> ist der Betrag, den du als Garage dafür bezahlst – die Differenz ist genau die Summe aus Aufbereitung, Garantie, Standzeit und Marge. Diese Spanne (typisch 10–20%) ist keine Willkür, sondern deine Kalkulation. Wer sie dem Kunden transparent zeigt, verhandelt souveräner als mit einer Zahl aus dem Bauch."
                  c={[<strong key="0" />, <strong key="1" />]}
                />
              </p>
            </div>
        ...
        <section id="vergleich" className="py-16 px-4 bg-white scroll-mt-20">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-2xl md:text-3xl font-bold text-neutral-900 mb-6">
              {t("Rechner vs. Eurotax, auto-i-dat & Gratis-Bewertungen")}
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <div className="bg-neutral-50 p-6 rounded-xl border border-neutral-200 shadow-sm">
                <h3 className="font-bold text-lg mb-2 text-neutral-900">{t("Katalog-Tools")}</h3>
                <p className="text-sm text-neutral-600">
                  {t("Eurotax & Co. liefern Richtwerte aus historischen Daten – im Abo oder pro Bewertung kostenpflichtig. Stark als Anker, aber Marktpreise schwanken je nach Region und Saison.")}
                </p>
              </div>
              <div className="bg-neutral-50 p-6 rounded-xl border border-neutral-200 shadow-sm">
                <h3 className="font-bold text-lg mb-2 text-neutral-900">{t("Gratis-Bewertungen")}</h3>
                <p className="text-sm text-neutral-600">
                  {t("Verbraucher-Tools schätzen den Verkaufspreis für Private – ohne Händler-Abzüge. Für den Ankauf fehlt der ganze Rechenweg von Marktwert zu Ankaufspreis.")}
                </p>
              </div>
              <div className="bg-red-50 p-6 rounded-xl border border-red-200 shadow-sm">
                <h3 className="font-bold text-lg mb-2 text-red-900">{t("Dieser Rechner")}</h3>
                <p className="text-sm text-red-900/80">
                  {t("Marktbasiert und für den Händler-Alltag gebaut: echte Inseratspreise rein, deine Kosten und Marge drauf – transparenter Eintauschwert raus.")}
                </p>
              </div>
            </div>
          </div>
        ...
        <section id="garagen" className="py-16 px-4 bg-gradient-to-br from-neutral-900 to-neutral-800 text-white scroll-mt-20">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-2xl md:text-3xl font-bold mb-4">{t("Für Garagen: vom Ankauf bis zum Verkauf")}</h2>
            <p className="text-lg text-neutral-300 mb-8 max-w-2xl mx-auto">
              {t("Im Garagen-Paket hast du den Rechner direkt in deinem Konto, ein festes Bewertungs-Kontingent pro Monat, inserierst Occasionen und bekommst eine eigene Garagen-Seite mit deinem ganzen Bestand.")}
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10 text-left">
              <div className="bg-white/10 p-6 rounded-xl border border-white/10">
                ...
                <h3 className="font-bold text-lg mb-2">{t("Sauber ankaufen")}</h3>
                <p className="text-sm text-neutral-400">{t("Eintauschwert marktbasiert berechnen – mit Rechenweg fürs Kundengespräch, auch im Dashboard.")}</p>
              </div>
              <div className="bg-white/10 p-6 rounded-xl border border-white/10">
                <ShieldCheck className="w-8 h-8 text-red-400 mb-4" />
                <h3 className="font-bold text-lg mb-2">{t("Bewertungen im Paket")}</h3>
                <p className="text-sm text-neutral-400">{t("25 bis 400 automatische Suchen pro Monat, je nach Garagen-Paket – manuelle Berechnungen immer ohne Limit.")}</p>
              </div>
              <div className="bg-white/10 p-6 rounded-xl border border-white/10">
                <Building2 className="w-8 h-8 text-red-400 mb-4" />
                <h3 className="font-bold text-lg mb-2">{t("Eigene Garagen-Seite")}</h3>
                <p className="text-sm text-neutral-400">{t("Dein ganzer Fahrzeugbestand auf einer eigenen Seite – zum Teilen, bei Google auffindbar, ohne Website-Projekt.")}</p>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Button asChild size="lg" variant="outline" className="border-white/20 hover:bg-white/10 hover:text-white bg-transparent text-white">
                <Link href="/preise">{t("Preise vergleichen")} <ArrowRight className="w-4 h-4 ml-2" /></Link>
              </Button>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-12">
              <h2 className="text-2xl md:text-3xl font-bold text-neutral-900 mb-3">
                {t("Häufige Fragen zum Eintauschwert")}
              </h2>
              <p className="text-neutral-600 text-lg">
                {t("Eintauschwert, Ankaufspreis & Occasionsbewertung – kurz erklärt")}
              </p>
            </div>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Link href="/auto-abo-vs-leasing-kosten" className="flex items-center justify-between gap-2 bg-neutral-50 rounded-xl border border-neutral-200 px-5 py-4 text-neutral-700 hover:border-red-300 hover:text-red-600 transition-colors group">
                <span className="font-medium">{t("Auto-Abo vs. Leasing: Kostenrechner")}</span>
              ...
              <Link href="/leasinguebernahme-kosten" className="flex items-center justify-between gap-2 bg-neutral-50 rounded-xl border border-neutral-200 px-5 py-4 text-neutral-700 hover:border-red-300 hover:text-red-600 transition-colors group">
                <span className="font-medium">{t("Was kostet eine Leasingübernahme?")}</span>
                <ChevronRight className="w-4 h-4 text-red-400 group-hover:translate-x-0.5 transition-transform" />
              </Link>
              <Link href="/suche?dealType=direct_purchase" className="flex items-center justify-between gap-2 bg-neutral-50 rounded-xl border border-neutral-200 px-5 py-4 text-neutral-700 hover:border-red-300 hover:text-red-600 transition-colors group">
                <span className="font-medium">{t("Occasionen auf BuyAuto durchsuchen")}</span>
                <ChevronRight className="w-4 h-4 text-red-400 group-hover:translate-x-0.5 transition-transform" />
              </Link>
              <Link href="/garage-plan" className="flex items-center justify-between gap-2 bg-neutral-50 rounded-xl border border-neutral-200 px-5 py-4 text-neutral-700 hover:border-red-300 hover:text-red-600 transition-colors group">
                <span className="font-medium">{t("BuyAuto für Garagen & Händler")}</span>
                <ChevronRight className="w-4 h-4 text-red-400 group-hover:translate-x-0.5 transition-transform" />
              </Link>
            </div>
          </div>
        ...
        <section id="disclaimer" className="py-12 px-4 bg-neutral-100 text-sm text-neutral-500">
          <div className="max-w-4xl mx-auto text-center space-y-2">
            <p>
              <T
                k="<0>Disclaimer:</0> Dieser Rechner dient als Orientierungshilfe und Modellrechnung. Der tatsächliche Wert eines Fahrzeugs hängt von Zustand, Ausstattung, Unfallhistorie, Nachfrage und Region ab und kann nur durch eine Begutachtung verbindlich bestimmt werden."
                c={[<strong key="0" />]}
              />
            </p>
            <p>{t("BuyAuto.ch übernimmt keine Gewähr für die Richtigkeit der Ergebnisse. Keine Kauf- oder Finanzberatung.")}</p>
          </div>
        </section>
      </main>
    </>
