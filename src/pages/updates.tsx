...
export default function UpdatesPage() {
  ...
  return (
    <>
      <Head>
        <title>{t("Was ist neu bei BuyAuto – Updates & neue Funktionen | BuyAuto")}</title>
        <meta
          name="description"
          content={t("Alle Neuerungen auf BuyAuto auf einen Blick: Eintauschwert-Rechner für Garagen, VIN-Decode ohne Konto, Käuferauswahl beim Verkauf, Message Center und mehr.")}
        />
        <link rel="canonical" href={absoluteUrl("/updates", locale)} />
        <meta property="og:title" content={t("Was ist neu bei BuyAuto – Updates & neue Funktionen")} />
        <meta
          property="og:description"
          content={t("Eintauschwert-Rechner, VIN-Decode ohne Konto, Käuferauswahl beim Verkauf und mehr – alle Neuerungen auf BuyAuto.")}
        />
        <meta property="og:type" content="website" />
        <meta property="og:url" content={absoluteUrl("/updates", locale)} />
      </Head>
        ...
        <main className="container py-10 sm:py-14">
          ...
          <header className="max-w-3xl">
            ...
            <h1 className="text-3xl md:text-5xl font-bold tracking-tight leading-tight text-neutral-900 mb-5">
              {t("Was ist neu bei BuyAuto")}
            </h1>
            <p className="text-lg text-neutral-600 leading-relaxed">
              {t("BuyAuto ist ein kleines Schweizer Projekt aus Schlieren – und wächst Schritt für Schritt. Hier findest du die Funktionen, die zuletzt dazugekommen sind, ohne Marketing-Nebel: was es macht, für wen es gedacht ist, und wo du es findest.")}
            </p>
          </header>
          ...
          <section className="mt-12 bg-white rounded-2xl shadow-lg border-2 border-primary p-6 md:p-10">
            ...
            <p className="text-neutral-700 leading-relaxed max-w-3xl">{t(HEADLINE.body)}</p>
            {HEADLINE.href && (
              <Button
                asChild
              ...
              >
                <Link href={HEADLINE.href}>
                  {HEADLINE.linkLabel && t(HEADLINE.linkLabel)}
          ...
          <section className="mt-12">
            <h2 className="text-2xl font-bold text-neutral-900 mb-6">{t("Ausserdem neu")}</h2>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <h3 className="font-bold text-neutral-900 text-lg">{t(update.title)}</h3>
                    </div>
                    <p className="text-neutral-600 leading-relaxed flex-1">{t(update.body)}</p>
                    {update.href && (
                      <Link
                        href={update.href}
                        className="mt-4 inline-flex items-center gap-1.5 text-primary font-semibold hover:underline"
                      >
                        {update.linkLabel && t(update.linkLabel)}
          ...
          <section className="mt-12 bg-primary/5 border-l-4 border-primary p-8 rounded-r-xl shadow-sm max-w-3xl">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="font-bold text-neutral-900 text-xl">{t("Kurz und ehrlich")}</h2>
            </div>
            <p className="text-neutral-700 leading-relaxed">
              {t("Hinter BuyAuto steckt aktuell ein One-Man-Team. Wenn dir etwas fehlt oder etwas nicht so funktioniert, wie du es erwartest, schreib mir einfach – das meiste hier oben ist genau so entstanden.")}
            </p>
          ...
          <section className="mt-12 text-center">
            <h2 className="text-2xl font-bold text-neutral-900 mb-4">{t("Bereit für dein Inserat?")}</h2>
            <p className="text-neutral-600 mb-6 max-w-xl mx-auto">
              {t("Fahrzeug erfassen, Angebotsart wählen, veröffentlichen – Leasingübernahme und Direktkauf laufen über denselben Flow.")}
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              ...
              >
                <Link href="/suche">{t("Fahrzeuge ansehen")}</Link>
              </Button>
            </div>
          </section>
        </main>
...
export const getStaticProps = async (context: GetStaticPropsContext) => {
  return { props: { ...(await withI18n(context.locale, ["pages/updates"])) }, revalidate: 300 };
};
