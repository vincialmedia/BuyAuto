...
function loadCatalog() {
  ...
  for (const file of files) {
    ...
    for (const r of raw.rename ?? []) {
      if (!r.from || !r.to) throw new Error(`${file}: rename needs "from" and "to"`);
      // Deliberately compares the raw strings, not the normalized ones. Most renames here are
      // cosmetic — "IONIQ 5 N" -> "Ioniq 5 N", "Megane RS" -> "Mégane RS" — and normalization
      // strips exactly the casing and accents those are fixing, so comparing normalized names
      // would reject the common case as a no-op.
      if (r.from === r.to) throw new Error(`${file}: rename "${r.from}" changes nothing`);
      renames.push({ make, ...r });
    }

    for (const m of raw.merge ?? []) {
      if (!m.from || !m.model) throw new Error(`${file}: merge needs "from" and "model"`);
      if (m.variant) {
        throw new Error(`${file}: merge "${m.from}" must not name a variant — use remap for that`);
      }
      merges.push({ make, ...m });
  ...
  return { groups, remaps, merges, renames, newModels, files, modelKeys, pairCount };
}
...
function buildSql({ groups, remaps, merges, renames, newModels }) {
  const out = [];
...
where mo.make_id = mk.id
  ...
  if (renames.length) {
    // The row is the right car at the right level, just under the wrong string: a generation
    // code that leaked out of the source ("Zafira Tourer C") or a second spelling of a name
    // already used elsewhere in the make. The row keeps its identity, listings and variants —
    // only the label changes — so this is neither a remap nor a merge.
    out.push('-- ---------- rename: generation codes and inconsistent spellings ----------');
    const rows = renames
      .map((r) => `(${sqlStr(r.make)},${sqlStr(r.from)},${sqlStr(r.to)})`)
      .join(',\n    ');
    out.push(`create temp table _rename on commit drop as
select * from (values\n    ${rows}\n) as t(make, from_name, to_name);`);
    out.push('');
    // Alias first: after the update the old spelling no longer matches anything.
    out.push(`insert into public.vehicle_aliases (entity_type, make_id, model_id, alias, normalized_alias, source)
select 'model', mk.id, mo.id, r.from_name, public.normalize_vehicle_name(r.from_name), ${sqlStr(SOURCE)}
from _rename r
join public.makes mk on mk.normalized_name = public.normalize_vehicle_name(r.make)
...
where not exists (
  ...
  where va.entity_type = 'model' and va.make_id = mk.id
    and va.normalized_alias = public.normalize_vehicle_name(r.from_name));`);
    out.push('');
    // If the target name is already taken the two rows are duplicates, which is a merge, not a
    // rename — skip rather than trip the unique index and take the whole transaction down.
    out.push(`update public.models mo set name = r.to_name, updated_at = now()
from _rename r
join public.makes mk on mk.normalized_name = public.normalize_vehicle_name(r.make)
where mo.make_id = mk.id
  and mo.normalized_name = public.normalize_vehicle_name(r.from_name)
  ...
  if (merges.length) {
    // A merge is a remap with nowhere to land: the row is a duplicate spelling of another
    // model ("Q6 SUV e-tron"), a generation artefact ("Passat V facelift") or an equipment
    // line ("Passat R-Line"), which the granularity contract keeps out of variants. The
    // text stays resolvable via the alias, but no variant row is invented for it.
    out.push('-- ---------- merge: duplicate, generation and equipment-line rows ----------');
    const rows = merges
      .map((m) => `(${sqlStr(m.make)},${sqlStr(m.from)},${sqlStr(m.model)})`)
      .join(',\n    ');
    out.push(`create temp table _merge on commit drop as
select * from (values\n    ${rows}\n) as t(make, from_name, model);`);
    out.push('');
    out.push(`insert into public.vehicle_aliases (entity_type, make_id, model_id, alias, normalized_alias, source)
select 'model', mk.id, mo.id, m.from_name, public.normalize_vehicle_name(m.from_name), ${sqlStr(SOURCE)}
from _merge m
...
where not exists (
  ...
  where va.entity_type = 'model' and va.make_id = mk.id
    ...
    out.push('');
    // variant_id is left alone: the listing keeps whatever engine it was filed under.
    out.push(`update public.listings l
set model_id = mo.id
from _merge m
join public.makes mk on mk.normalized_name = public.normalize_vehicle_name(m.make)
...
const stats =
  ...
  `${catalog.remaps.length} remaps, ${catalog.merges.length} merges, ` +
  `${catalog.renames.length} renames, ${catalog.files.length} make files`;
