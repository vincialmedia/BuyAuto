-- The Leasingübernahme checklist fired only for deal_type = 'lease_takeover'
-- and missed the hybrid "Direktkauf + Leasingübernahme" listings, where the
-- takeover lives in leasing_offer -> lease_takeover_offer (enabled: true)
-- while deal_type stays 'direct_purchase' (that flag is how the wizard and
-- the listing UI detect the hybrid too, see ListingWizard.tsx).
--
-- select_buyer_and_mark_listing_sold is restated in full (CREATE OR REPLACE
-- swaps the whole body) with ONLY the takeover gate widened; everything else
-- is identical to 20260804190000_buyer_selected_workflow.sql. The
-- buyer-selected-notification edge function carries the same widened check
-- for the next-steps emails.
