...
function buildEmail(params: {
  ...
  return {
    subject: `⏳ Ihr Abo endet in ${params.daysBefore} Tagen`,
    html: `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  ...
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="card">
        ...
        <p style="margin: 10px 0 0 0;">Abo-Ende: <strong>${escapeHtml(endsDate)}</strong></p>
      </div>

      <p style="margin-top: 18px;">
        Ihr Abo endet in <strong>${params.daysBefore} Tagen</strong>. Öffnen Sie Ihr Dashboard, um Ihren Plan zu verwalten.
      </p>
...
serve(async (req) => {
  ...
  for (const daysBefore of daysBeforeList) {
    ...
    const { data: subs, error: subsError } = await supabase
      ...
      .select("id,dealer_id,plan_id,status,current_period_end,grace_ends_at")
      .eq("status", "active")
    ...
    if (subsError) {
      console.error("subscription-expiry-reminder subsError:", subsError);
      results.push({ daysBefore, scanned: 0, sent: 0, skipped: 0, errors: 1 });
      continue;
    }

    const rows: SubscriptionRow[] = Array.isArray(subs) ? (subs as SubscriptionRow[]) : [];
    const dealerIds = Array.from(new Set(rows.map((r) => r.dealer_id)));
    const planIds = Array.from(
      new Set(rows.map((r) => r.plan_id).filter((v): v is string => typeof v === "string" && v.length > 0))
    );
    ...
    if (dealerIds.length > 0) {
      ...
      if (garagesError) console.error("subscription-expiry-reminder garagesError:", garagesError);
    ...
    if (planIds.length > 0) {
      ...
      if (plansError) console.error("subscription-expiry-reminder plansError:", plansError);
    ...
    if (ownerIds.length > 0) {
      ...
      if (profilesError) console.error("subscription-expiry-reminder profilesError:", profilesError);
    ...
    for (const s of rows) {
      const garage = garagesById.get(s.dealer_id);
      ...
      if (logError) {
        ...
        console.error("subscription-expiry-reminder log insert error:", logError);
        errors++;
        continue;
      }

      const plan = s.plan_id ? plansById.get(s.plan_id) : undefined;
      ...
      const emailPayload = buildEmail({
        ...
        garageName,
        planName,
        endsAtIso: s.current_period_end,
        daysBefore,
        dashboardUrl,
        pricingUrl,
      });
      ...
      if (sendRes.error) {
        console.error("subscription-expiry-reminder resend error:", sendRes.error);
        errors++;
        continue;
      }
